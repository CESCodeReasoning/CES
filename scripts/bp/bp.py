...
from prompt import chatgpt_generator, huggingface_generator, huggingface_generator_chat, generator_lllm, generator_gemini
import json
...
def main(model_id, dataset, cache_dir, write_dir, with_tests):
    ...
    if with_tests:
        write_root = os.path.join(write_dir, 'BP', model_id.split("/")[-1], dataset+"-tests")
    else:
    ...
    for d in tqdm(os.listdir(root_dir)):
        ...
        write_path_buggy = os.path.join(write_folder, 'response_1.txt')
        write_path_fix = os.path.join(write_folder, 'response_0.txt')
        if os.path.exists(write_path_buggy) and os.path.exists(write_path_fix):
            continue
        ...
        ## prompt fixed code:
        prompt = create_prompt(model_id, fixed_code, entry_point, tests, nl)
        if api_type=="openai":
            err_flg, response = chatgpt_generator(model_id, prompt)
            if err_flg:
                response = 'ERR: reaches maximum context length'
        ...
        if api_type == "gai":
            response = generator_gemini(model_id, prompt)
        with open(write_path_fix, 'w') as wr:
            wr.write(response)
